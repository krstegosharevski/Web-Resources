...
axm.TravelRequest.Event = (function () {
    ...
    async function fetchEntityDefinitions(logicalName, selectedParams) {
        const res = await fetch(`/api/data/v9.2/EntityDefinitions(LogicalName='${logicalName}')?$select=${selectedParams}`); // kk msm deka mora da ima tuka permisii
    ...
    async function createAndRefresh(recordName, data, pControl) {
        try {
            var newReservation = await Xrm.WebApi.createRecord(recordName, data); // kk
            var editStrings = {
